...
def generate_control_file(version):
    ...
    content = """Package: dugadesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: DugaDesk <info@duga.pw>
Homepage: https://duga.pw
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/dugadesk')
    system2('mkdir -p tmpdeb/etc/dugadesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/dugadesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/dugadesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/dugadesk/')
    system2(
        'cp ../res/dugadesk.service tmpdeb/usr/share/dugadesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/dugadesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/dugadesk.svg')
    system2(
        'cp ../res/dugadesk.desktop tmpdeb/usr/share/applications/dugadesk.desktop')
    system2(
        'cp ../res/dugadesk-link.desktop tmpdeb/usr/share/applications/dugadesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/dugadesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/dugadesk/')
    system2(
        'cp ../res/pam.d/dugadesk.debian tmpdeb/etc/pam.d/dugadesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/dugadesk/files/polkit && chmod a+x tmpdeb/usr/share/dugadesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb dugadesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('dugadesk.deb', '../dugadesk-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/dugadesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/dugadesk/')
    system2(
        'cp ../res/dugadesk.service tmpdeb/usr/share/dugadesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/dugadesk.png')
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/DugaDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"DugaDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon DugaDesk.app 200 190 --hide-extension DugaDesk.app dugadesk.dmg ./build/macos/Build/Products/Release/DugaDesk.app")
    os.rename("dugadesk.dmg", f"../dugadesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    os.chdir('../..')
    if os.path.exists('./dugadesk_portable.exe'):
        os.replace('./target/release/rustdesk-portable-packer.exe',
                   './dugadesk_portable.exe')
    else:
        os.rename('./target/release/rustdesk-portable-packer.exe',
                  './dugadesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/dugadesk_portable.exe')
    os.rename('./dugadesk_portable.exe', f'./dugadesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/dugadesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/dugadesk-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/dugadesk-{version}-win7-install.exe ../..')
        ...
        system2('mv dugadesk-%s-0-x86_64.pkg.tar.zst dugadesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/dugadesk-%s-0.x86_64.rpm ./dugadesk-%s-fedora28-centos8.rpm' % (
                version, version))
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/dugadesk-%s-0.x86_64.rpm ./dugadesk-%s-suse.rpm' % (
                version, version))
        # yum localinstall rustdesk.rpm
    else:
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/DugaDesk.app/Contents/MacOS/rustdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/DugaDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/DugaDesk.app/Contents/MacOS/rustdesk
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/DugaDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/DugaDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/DugaDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/DugaDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "DugaDesk %s.dmg" "target/release/bundle/osx/DugaDesk.app"' % version)
                os.rename('DugaDesk %s.dmg' %
                          version, 'dugadesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./dugadesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./dugadesk-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple dugadesk-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/DugaDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/dugadesk*.deb ./dugadesk.deb')
                system2('dpkg-deb -R dugadesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/dugadesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/dugadesk-link.desktop tmpdeb/usr/share/applications/dugadesk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/dugadesk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/dugadesk/')
                os.system('mkdir -p tmpdeb/etc/X11/dugadesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/dugadesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/dugadesk.debian tmpdeb/etc/pam.d/dugadesk')
                system2('strip tmpdeb/usr/bin/dugadesk')
                system2('mkdir -p tmpdeb/usr/share/dugadesk')
                system2('mv tmpdeb/usr/bin/dugadesk tmpdeb/usr/share/dugadesk/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/dugadesk/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb dugadesk.deb; /bin/rm -rf tmpdeb/')
                os.rename('dugadesk.deb', 'dugadesk-%s.deb' % version)
