...
> ## НИ ОДНА СБОРКА ЕЩЁ НЕ ЗАПУСКАЛАСЬ
>
> Слой проверен чтением файлов и прогоном `apply.py` (якоря, идемпотентность,
> разбор YAML/JSON/XML/Python, граф зависимостей job'ов). **Ни один artefact
> не собран ни на одной платформе**, CI не запускался ни разу. До первого
> зелёного прогона считать слой НЕПРОВЕРЕННЫМ вживую: подтверждены синтаксис
> и логика, но не поведение сборки.

```
brand/
  brand.toml   параметры продукта (имя, домен, ключ, версия) — единственный источник истины
  apply.py     применитель: 191 правило, всё по текстовым якорям
...
## Что слой меняет (14 разделов, 191 правило)
